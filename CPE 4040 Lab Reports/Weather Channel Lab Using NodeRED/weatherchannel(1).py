...
def get_data_aws(s3bucket):
    ...
    obj = s3.Object(AWS_S3_BUCKET,AWS_S3_KEY_PREFIX)
...
while True:
    ...
    current_temp = float(df['temp'].tail(1))
    current_humidity = float(df['humidity'].tail(1))
    ...
    with placeholder.container():
        ...
        metric3.metric(
			...
			value = round(pd.to_numeric(df['temp']).mean(),2))
